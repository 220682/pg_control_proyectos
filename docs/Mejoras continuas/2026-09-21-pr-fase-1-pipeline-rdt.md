...
**Estado (cierre de sesión, 2026-09-21):** PLAN completo, sin implementar. Trabaja en paralelo con [Fase 2](2026-09-21-pr-fase-2-pipeline-linea-base.md) (Agente B).

**Pendiente antes de arrancar:**
1. Victor aprueba la Punch List de esta fase (19 ítems, tabla más abajo) — "PR Fase 1 — pipeline RDT → PR".
2. Victor corre en Supabase la migración `db/052_pr_enriquecido.sql` (ya escrita en `py_control_proyectos_web`, contrato compartido con la Fase 2, idempotente).
3. Coordinar con el Agente B el orden de migraciones: la `053` de esta fase va antes que la `060` de la Fase 2 (ver "Qué puede romperse").
